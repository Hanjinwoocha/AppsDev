...
namespace EnrollmentSystem.Repo
{
    internal class EnrollmentEntryData
    {
                ...
                using (OleDbCommand preqCmd = new OleDbCommand(preqQuery, conn))
                {
                    ...
                    if (preqResult != null && preqResult != DBNull.Value)
                    {
                        ...
                        using (OleDbCommand gradeCmd = new OleDbCommand(gradeQuery, conn))
                        {
                            ...
                            if (gradeResult == null || gradeResult == DBNull.Value || gradeResult.ToString().ToUpper() == "NOT GRADED")
                            {
                                message = $"Cannot enroll: prerequisite subject ({preqCode}) has not been graded yet.";
                                return false;
                            }
                            if (gradeResult.ToString().ToUpper() == "FAILED")
                            {
        ...
        public bool IsStudentAlreadyEnrolledInEDP(int studentId, string edpCode)
        {
            using (OleDbConnection conn = new OleDbConnection(connectionString))
            {
                conn.Open();
                string query = "SELECT COUNT(*) FROM EnrollmentDetailFile WHERE ENRDFSTUDID = ? AND ENRDFSTUDEDPCODE = ? AND ENRDFSTUDSTATUS = 'EN'";
                using (OleDbCommand cmd = new OleDbCommand(query, conn))
                {
                    cmd.Parameters.AddWithValue("@ENRDFSTUDID", studentId);
                    cmd.Parameters.AddWithValue("@ENRDFSTUDEDPCODE", edpCode);
